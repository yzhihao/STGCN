...
class Dataloader():
    ...
    def load_data(self, data_file,data_label_file,vocab_dict,sen_list,bert_seq):
        # Load  train data
        examples = []
        label_examples = []
        ...
        self.bert_seq_batches = np.split(self.bert_seq, self.num_batch, 0)
        ...
        self.bert_seq_test = np.array(bert_seq[TRAIN_SIZE:])
        ...
        self.bert_seq_test = self.bert_seq_test[:self.num_batch_test * self.batch_size]
        ...
        self.bert_seq_test_batches = np.split(self.bert_seq_test, self.num_batch_test, 0)
        self.pointer_test = 0


    def next_batch(self):
        ret = self.sentences_batches[self.pointer], self.labels_batches[self.pointer],self.sen_emb_batches[self.pointer],self.bert_seq_batches[self.pointer]
        self.pointer = (self.pointer + 1) % (self.num_batch)
        return ret

    def test_batch(self):#Preserve part of dataset for testing
        ret = self.sentences_test_batches[self.pointer_test], self.labels_test_batches[self.pointer_test],self.sen_emb_test_batches[self.pointer_test],self.bert_seq_test_batches[self.pointer_test]
...
class Detection:
    def __init__(self, sequence_length, batch_size, vocab_size, emb_dim, hidden_dim=128, output_keep_prob=0.7):
        ...
        with tf.variable_scope("placeholder"):
            ...
            self.bert = tf.placeholder(shape=[self.batch_size,768], dtype=tf.float32)

            #self.x1 = tf.concat([self.bert, self.x], 2)
            #self.x1=self.bert
        ...
        with tf.name_scope('dropout'):
            # dropout防止过拟合
            sentence_representation = tf.concat([sentence_representation, self.bert,self.x_sen], 1)  # [64,456]和句子向量拼接
        ...
        with tf.name_scope('output'):
            ...
            W = tf.get_variable(shape=[hidden_dim * 2+768+EMB_DIM, self.output_dim],initializer=tf.contrib.layers.xavier_initializer(),name='W')
        ...
        with tf.variable_scope("train"):
            ...
            self.train_op = tf.train.AdamOptimizer(0.0001).apply_gradients(gradients)
    ...
    def train(self, sess, x_batch, y_batch,x_sen,bert_seq):
        _, loss = sess.run([self.train_op, self.loss], feed_dict={self.x: x_batch, self.targets: y_batch, self.x_sen: x_sen,self.bert:bert_seq})
        return loss
    ...
    def get_accuracy(self, sess, x_batch, y_batch,x_sen,bert_seq):
        accuracy = sess.run([self.accuracy], feed_dict={self.x: x_batch, self.targets: y_batch, self.x_sen: x_sen,self.bert:bert_seq})
...
if __name__ == "__main__":
    ...
    ''''''
    bert_seq=np.load("mr_bert_seq.npy")

    dis_data_loader.load_data(data_file,data_label_file,vocab_dict,sen_list,bert_seq)
    ...
    with tf.Session() as sess:
        ...
        for i in range(STEPS):
            x_batch, y_batch,x_sen,bert_seq= dis_data_loader.next_batch()
            loss = detection.train(sess, x_batch, y_batch,x_sen,bert_seq)
            loss_all=loss_all+loss
            if (i % 100 == 0):
                accuracy_list=[]
                for j in range(dis_data_loader.num_batch_test):
                    test_x_batch, test_y_batch,x_test_sen,bert_seq = dis_data_loader.test_batch()
                    accuracy_list.append(detection.get_accuracy(sess, test_x_batch, test_y_batch,x_test_sen,bert_seq))
